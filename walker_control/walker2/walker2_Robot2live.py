...
# 时间：20231030
# 功能：2> 创建一个节点，订阅一个话题，在回调函数中 对接收到的/walker/leftLimb/joint_states话题数据进行处理，
# 然后对数据进行处理转发发布到另一个话题/walker2/joint_command。
# 1106新增：针对仿真环境下的walker2进行修改
...
def transit():
    ...
    global pub
    pub = rospy.Publisher('walker2/joint_command', JointState, queue_size=10)
    real_flag = True
    if real_flag:
        # 在walker2真机上--腿、手掌、头和仿真环境下walkerX不同
        rospy.Subscriber('/Leg/MeasuredJoint', JointState, update_realLeg_data) # noqa
        rospy.Subscriber('/walker/leftLimb/joint_states', JointState, update_reallLimb_data) # noqa
        rospy.Subscriber('/walker/rightLimb/joint_states', JointState, update_realrLimb_data) # noqa
